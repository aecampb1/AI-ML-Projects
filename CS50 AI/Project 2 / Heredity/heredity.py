    ...
    for have_trait in powerset(names):
        ...
        for one_gene in powerset(names):
            for two_genes in powerset(names - one_gene):
                ...
                update(people, probabilities, one_gene, two_genes, have_trait, p)
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in one_gene:
        if people[person]["mother"] is None:
            ...
            if mother in one_gene and father in two_genes:
                probability *= 0.5 * PROBS["mutation"] + (1 - PROBS["mutations"]) * 0.5
        ...
        # Calculate the probability of exhibiting the trait given the gene count
    ...
    for person in two_genes:
        if people[person]["mother"] is None:
            ...
            if mother in one_gene and father in two_genes:
                probability *= 0.5 * (1 - PROBS["mutation"])
            if mother in two_genes and father in zero_genes:
                probability *= (1 - PROBS["mutation"]) * PROBS["mutation"]
            if mother in two_genes and father in one_gene:
                probability *= (1 - PROBS["mutation"]) * 0.5
            if mother in two_genes and father in two_genes:
        ...
        # Accumulate the joint probability for the entire family
        probability *= PROBS["trait"][2][person in have_trait]

        print(f"Person: {person}, Gene Count: {gene_count}, Gene Prob: {gene_prob}, Trait Prob: {trait_prob}, Joint Prob: {joint_prob}")
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        ...
        # Check if person has one copy of the gene or two copies of the gene
...
def normalize(probabilities):
    ...
    """
    for person in probabilities:
        # Calulate gene_sum and trait_sum as the sum of probabilties before normalization
        gene_sum = sum(probabilities[person]["gene"].values())
        trait_sum = sum(probabilities[person]["trait"].values())

        # Normalize gene distribution
        for gene_count in probabilities[person]["gene"]:
            probabilities[person]["gene"][gene_count] /= gene_sum if gene_sum != 0 else 1

        # Normalize trait distribution
        for trait_value in probabilities[person]["trait"]:
            probabilities[person]["trait"][trait_value] /= trait_sum if trait_sum != 0 else 1
